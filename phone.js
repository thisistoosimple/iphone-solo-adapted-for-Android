...
function createPhoneScene(canvas) {
  ...
  function onMotion(e) {
    ...
    // Many Android devices never expose a linear-acceleration sensor,
    // so `e.acceleration` stays null forever there. When it's missing we
    // fold it straight into the total instead of bailing out, which is a
    // touch noisier during a fast roll but keeps the page working.
    const linear = e.acceleration;
    const x = linear ? total.x - linear.x : total.x;
    const z = linear ? total.z - linear.z : total.z;
  ...
  async function enableMotion(fromTap = false) {
    ...
    try {
      if (typeof DeviceMotionEvent.requestPermission === 'function') {
        ...
        if (state !== 'granted') {
          showUnavailable('Motion access is off. Allow it for this page in your browser settings, then try again.', enableMotion);
          return;
        }
      }
